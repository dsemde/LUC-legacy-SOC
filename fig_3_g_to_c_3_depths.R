...
dat <- combined |>
  mutate(history = 
           case_when(history == "only_C" ~ 
                     ...
                     !!!conditions)) |> 
  mutate(years_cont = 
           case_when(history == "Permanent grassland" ~ 
                       max_yc,
                     history == "Permanent cropland" ~ 
                       min_yc,
                     !!!years_cont)) |> 
  mutate(left = 
           case_when(history == "Permanent grassland" ~ 
...
dat_w <- foreach(i = seq_along(depths), .combine = rbind, .packages = c("twang", "WeightIt")) %dopar% {
...
plot_dat <- dat_w |> 
  ...
  mutate(facet = case_when(years_cont == max_yc ~ "Grassland", 
                           years_cont == min_yc ~ "Cropland",
                           TRUE ~ "Transitional"))
...
results <- foreach(i = seq_along(inter), .combine = rbind, .packages = c("boot", "dplyr")) %:%
...
## Calculate the boostrapped pvalue between cropland each LU interval
# cl <- makeCluster(n_cores, type = "PSOCK")
# registerDoParallel(cl)
#
# set.seed(123)
#
# cropdf <- dat_w |>
#     filter(history == "Permanent cropland")
#
# pvals <- foreach(i = 1:length(depths), .combine = rbind, .packages = c("boot", "boot.pval", "dplyr")) %:% 
#                 foreach(j = 1:length(inter), .combine = rbind) %dopar% { 
#
#     if (n_distinct(cropdf$depth_increment) > 1) {
#         crop <- cropdf |>
#             filter(depth_increment == depths[i])
#
#         cropboot <- boot(crop, mean.fun, R = 10000)
#     } else if (cropboot$data$depth_increment[1] != crop$depth_increment) {
#         crop <- cropdf |>
#         filter(depth_increment == depths[i])
#
#         cropboot <- boot(crop, mean.fun, R = 10000)
...
#                           depth_increment = depths[i],
#                           pvalue = boot.pval(comparison, theta_null = mean(cropboot$t0))) 
#     results
# }
#
# stopCluster(cl)
...
plot_dat <- plot_dat |> 
  mutate(new_left = ifelse(years_cont %in% unique(years_cont)[1:(length(unique(years_cont)) - 2)],  new_left, new_left + 6)) |>
  mutate(new_right = ifelse(years_cont %in% unique(years_cont)[1:(length(unique(years_cont)) - 3)],  new_right - 6, new_right))


# Adjust spacing for readability of LU interval 1
plot_dat2 <- plot_dat |>  
  mutate(new_left = ifelse(years_cont != unique(years_cont)[1],  new_left + 2, new_left)) |>
  mutate(new_right = new_right + 2) |> 
  # The years_cont variable is used for error bar placement, need to shift
  mutate(years_cont = ifelse(years_cont == unique(years_cont)[1], unique(years_cont)[1] - 1, years_cont))
...
p <- plot_dat |> 
  ...
  geom_hline(yintercept = h_line[["30-100cm"]], 
             ...
             color = h_line_col) +
  xlab(element_blank()) +
  ylab(expression(SOC~density~(Mg~ha^{-1}~cm^{-1}))) +
  ylim(-0.1, y_limit) +
  theme_minimal() +
...
# Set x-axis scale and labels based on LUC intervals
x_scale <- c(0, 10, 25, 32, 54, 101, 234)
x_labels <- c("0", "11", "26", "33", "55", "102", "230")
...
# Generate "n" annotation for number of sites per category
f2_annotation <- plot_dat2 |> 
  filter(facet == "Transitional") |>
  group_by(years_cont) |> 
  mutate(x = years_cont + 0.5,
...
p4 <- ggarrange(p1, p2, p3,
  ...
  theme(plot.margin = margin(b = -25, r = 8))

annotate_figure(p4, 
                bottom = text_grob("Years since conversion to cropland (from grassland)",
                                   family = "Montserrat",
...
ggsave(filename = here("figures",
                       "fig_3_grass_crop_luc_intervals_final.tiff"),
       width = 27,
       height = 18,
       units = "cm",
       bg = "white",
...
ggsave(filename = here("figures",
                       "fig_3_grass_crop_luc_intervals_final.png"),
       width = 27,
       height = 18,
       units = "cm",
       bg = "transparent",
