...
### Aggregate lab data to two depth increments ---------------------------------
...
hist_reduced <- hist |> 
    ...
    mutate(history = case_when((init.use == "G" & curr.use == "A" & use.changes == 1) ~ "G_to_C",
                                (init.use == "A" & curr.use == "A" & use.changes == 0) ~ "only_C",
                                (init.use == "G" & curr.use == "G" & use.changes == 0) ~ "only_G")) |>
    filter((history == "G_to_C" & change == 1) |
             history == "only_C" |
             history == "only_G") |>
    ungroup() |> 
    # Carry forward only years since the first conversion of G to A
    mutate(min_year = min(year[history == "G_to_C"])) |>
    filter(year >= (min(min_year) - 1)) |>
    # Create year of change variable
    group_by(PointID) |> 
    mutate(year_change = ifelse(history == "G_to_C", min(year), NA)) |>
    slice(1) |>
    ungroup()
...
write.csv(combined,
    file = "./dat/derived/g_to_c_3_depths.csv",
    row.names = FALSE)