...
dat_w <- foreach(i = 1:length(depths), #nolint
                 .combine = rbind, 
                 .packages = c("twang")) %dopar% {
...
regression_dat <- dat_w %>% 
  ...
  summarise(n = n_distinct(PointID),
            ...
            x = case_when(years_cont == 2000 ~ 400,
                          years_cont == 2012 ~ 0,
                          TRUE ~ years_cont)) %>%
  slice(1) %>% 
  ungroup() %>% 
  ...
  group_by(years_cont) %>%
  mutate(temp_mean = 
           (mean[depth_increment == "0-10cm"] + 
            mean[depth_increment == "10-30cm"] * 2) / 3,
         sd = 
           (sd[depth_increment == "0-10cm"] + 
            sd[depth_increment == "10-30cm"] * 2) / 3) %>%
  ungroup() %>% 
  summarise(y = ((temp_mean - temp_mean[years_cont == 2012]) / 
                   temp_mean[years_cont == 2012]) * 100,
            x = x,
            sd = sd,
            n = n) %>% 
  group_by(x) %>%
...
c_start <- min(regression_dat$y)
k_start <- 2 * abs(log(2)) / abs(c_start)
...
# Find SOC change at 20 years after LUC including CI
get_y(20, coef)
...
get_asymp_date <- function(coefs) {
  ...
  for (i in 1:max(regression_dat$x)) {
    asymp_date <- ifelse((abs(get_y((i + 1), coefs)) - abs(get_y(i, coefs))) <= 
                           asymp_cutoff, i + 1, 0)
...
ggplot() +
  ...
  annotate("text",
           x = 300, y = 15, label = "y == -33.57 (1 - e^{-0.01522*t})",
           parse = TRUE,
           size = 5,
           colour = "#4b4b4b",
           family = "Calibri") +
  ...
  labs(x = "Years since conversion to cropland (from grassland)",
...
ggsave(filename = here("figures",
                       "fig_4_grass_crop_soc_regression.png"),
       width = 27,
       height = 18,
       units = "cm",
       bg = "transparent",
