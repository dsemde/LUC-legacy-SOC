...
### Generate Propensity Score Weighting model ----------------------------------
...
dat_w <- foreach(i = seq_along(depths),
                 .combine = rbind,
                 .packages = "twang") %dopar% {
...
regression_dat <- dat_w |>
  ...
  summarise(y = ((temp_mean - temp_mean[years_cont == max_yc]) /
                   temp_mean[years_cont == min_yc]) * 100,
            x = x,
            sd = sd,
            n = n) |>
  group_by(x) |>
...
ggplot() +
  ...
  annotate("text",
           x = 300, y = 15, label = paste0("y == ", round(coef[1], 2), " (1 - e^{ -", round(coef[2], 5), "*t})"),
           parse = TRUE,
           size = 12,
           colour = "#4b4b4b",
           family = "Montserrat") +
...
ggsave(filename = here("figures",
                       "fig_4_grass_crop_soc_regression_final.tiff"),
       width = 27,
       height = 18,
       units = "cm",
       bg = "white",
...
ggsave(filename = here("figures",
                       "fig_4_grass_crop_soc_regression_final.png"),
       width = 27,
       height = 18,
       units = "cm",
       bg = "transparent",
