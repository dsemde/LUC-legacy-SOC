...
#                     depth_increment = depths[i],
#                     pvalue = boot.pval(comparison, theta_null = mean(grassboot$t0))) 
#     results
# }
...
plot_dat <- plot_dat |> 
  mutate(new_left = ifelse(years_cont %in% unique(years_cont)[1:(length(unique(years_cont)) - 2)], 
                           new_left, 
                           new_left + 6)) |> 
  mutate(new_right = ifelse(years_cont %in% unique(years_cont)[1:(length(unique(years_cont)) - 3)], 
                            new_right - 6, 
                            new_right))
...
p4 <- ggarrange(p1, p2, p3,
  ...
  theme(plot.margin = margin(b = -25, r = 10))

annotate_figure(p4, 
                bottom = text_grob("Years since conversion to grassland (from cropland)",
                                   family = "Montserrat",
...
ggsave(filename = here("figures",
                       "fig_5_crop_grass_luc_intervals_final.tiff"),
       width = 27,
       height = 18,
       units = "cm",
       bg = "white",
...
ggsave(filename = here("figures",
                       "fig_5_crop_grass_luc_intervals_final.png"),
       width = 27,
       height = 18,
       units = "cm",
       bg = "transparent",
