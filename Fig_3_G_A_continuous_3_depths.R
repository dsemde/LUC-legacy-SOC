...
### Load dataset ---------------------------------------------------------------
source(here("G_to_A_PS_data_3_depths.R")) #nolint

dat <- combined  %>% 
  filter(history == "G_to_A" & depth_increment == "0-10cm")


### Generate year intervals-----------------------------------------------------

n <- 6
# interval_list <- c(1782, 1913, 1962, 1978, 1990, 2002, 2011)
interval_list <- c()
...
#                           depth_increment = depths[i],
#                           pvalue = boot.pval(comparison, theta_null = mean(cropboot$t0))) #nolint
#     results
# }
...
plot_dat <- plot_dat %>% 
  mutate(new_left = ifelse(years_cont %in% unique(years_cont)[1:(length(unique(years_cont)) - 2)],  new_left, new_left + 6)) %>% #nolint
  mutate(new_right = ifelse(years_cont %in% unique(years_cont)[1:(length(unique(years_cont)) - 3)],  new_right - 6, new_right)) #nolint


# Adjust spacing for readability of LU interval 1
plot_dat2 <- plot_dat %>%  
  mutate(new_left = ifelse(years_cont != unique(years_cont)[1],  new_left + 2, new_left)) %>% #nolint
  mutate(new_right = new_right + 2) %>% 
  # The years_cont variable is used for error bar placement, need to shift
  mutate(years_cont = ifelse(years_cont == unique(years_cont)[1], unique(years_cont)[1] - 1, years_cont)) #nolint
...
annotate_figure(p4, 
                bottom = text_grob("Years since conversion to cropland (from grassland)", #nolint
                                   family = "Calibri"))
...
ggsave(filename = here("figures",
                       "fig_3_grass_crop_luc_intervals.png"),
       width = 27,
       height = 18,
       units = "cm",
       bg = "white",
